...
def get_inverse_features(features):

    inverse_features = []
    for x in features:
        feature_inverse = copy(x)
        feature_inverse.inverse = 1
        inverse_features.append(feature_inverse)
...
def init_simple():
    ...
    # Initialize all features with inverse = 0, then second half is copy of first half with inverse = 1
    ...
    features.extend(get_inverse_features(features))
